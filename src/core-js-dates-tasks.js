...
function getTime(date) {
  ...
  return `${hours.toString().length === 2 ? hours : `0${hours}`}:${minutes.toString().length === 2 ? minutes : `0${minutes}`}:${seconds.toString().length === 2 ? seconds : `0${seconds}`}`;
}
...
function getNextFriday(date) {
  const copyDate = date;
  const currentDay = copyDate.getDay();
  const fridayNum = 5;
  const fullWeek = 7;
  const diff =
    fridayNum - currentDay > 0
      ? fridayNum - currentDay
      : fridayNum + fullWeek - currentDay;

  return new Date(copyDate.setDate(copyDate.getDate() + diff));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = (data2 - data1) / (24 * 60 * 60 * 1000) + 1;
  return diff;
}
...
function isDateInPeriod(date, period) {
  ...
  const dataStart = new Date(period.start).getTime();
  const dataEnd = new Date(period.end).getTime();
  return currentDate >= dataStart && currentDate <= dataEnd;
}
...
function formatDate(date) {
  ...
  const dateNum = givenDate.getUTCDate();
  let hour = givenDate.getUTCHours();
  let minutes = givenDate.getUTCMinutes();
  let sec = givenDate.getUTCSeconds();
  let meridiem;
  if (hour < 12) {
    meridiem = 'AM';
  ...
  return `${month}/${dateNum}/${year}, ${hour}:${minutes}:${sec} ${meridiem}`;
}
...
function getQuarter(date) {
  const copyDate = new Date(date).getMonth();
  if (copyDate <= 3) {
    return 1;
  }
  if (copyDate <= 6) {
